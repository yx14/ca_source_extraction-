...
#%%
def HALS_initialization(Y, nr=30):
    ...
    T = sizY[len(sizY) - 1]
    dx = sizY[0:dimY]
    d = reduce(lambda x, y: x*y, dx)
    
    #median at entry i,j (k) over time
    med = []
    if dimY == 2:
        for i in range(sizY[0]):
            templist = []
            for j in range(sizY[1]):
                templist.append(np.median(Y[i, j, :]))
            med.append(templist)
    elif dimY == 3:
        for i in range(sizY[0]):
            templist = []
            for j in range(sizY[1]):
                templist2 = []
                for k in range(sizY[2]):
                    templist2.append(np.median(Y[i, j, k, :]))
    ...
    elif init_hals_method.lower() == 'cor_im':
        sk = np.maximum(round(T/1000), 1);
        Cn = cse.utilities.local_correlations(Y[:, :, 1:sk:T])
        Y = np.reshape(Y, (d, T))
        Cnf = nd.filters.gaussian_filter(Cn, 2.25)
        BW = nd.filters.maximum_filter(Cnf, 3)
        BW = (Cnf == BW)
        C = Y[np.ndarray.flatten(BW), :]
    ...
    #ADDED 
    b = np.reshape(med, [med.shape[0]*med.shape[1], 1])
    ...
    for iter in range(max_iter_hals_in):
        ...
        for i in range(A.shape[1]):
            A[A[:, i] < th*max(A[:, i]), i] = 0
        C = HALS_temporal(Y, A, C, 1)
    
    
    
    ind_del = np.std(C, 1) == 0
    A = np.delete(A, ind_del, axis = 1)
    C = np.delete(C, ind_del, axis = 0) 
    
    Y = np.add(Y - np.dot(A, C), np.reshape(med, [med.shape[0]*med.shape[1], 1]))
    ...
    for iter in range(max_iter_hals_in):
        ...
        b = np.maximum(np.dot(Y, np.transpose(f))/np.linalg.norm(f)**2, 0)
        
    '''
    return [A, C, b, f]
#HALS_spatial returns A, does not currently take activepixel or maxIter
def HALS_spatial(Y, A, C):
...
def HALS_temporal(Y, A, C, maxIter):
    K = A.shape[1]
    U = np.dot(np.transpose(A), Y)
    V = np.dot(np.transpose(A), A)
    aa = np.diag(V)
    
    for miter in range(maxIter):
        for k in range(K):
            ck = C[k, :] + (U[k, :] - np.dot(V[k, :], C) )/aa[k]
            low_vals = ck < 0
            ck[low_vals] = 0
            C[k, :] = ck
